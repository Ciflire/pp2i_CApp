...
def cleanup():
    ...
    # somme des bornes par coordonnes et puissance
    XYPN = XYPN.groupby(["X", "Y", "puissance_nominale"]).max()
    ...
    XYPN['puissance_nominale'] = np.where(
        XYPN['puissance_nominale'] >= 1000, XYPN['puissance_nominale'] // 1000, XYPN['puissance_nominale'])
    XYPN['puissance_nominale'] = np.where(
        XYPN['puissance_nominale'] >= 250, XYPN['puissance_nominale'] // 2, XYPN['puissance_nominale'])
    ...
    for i in range(1, len(XYPN.index)):
        new_line = haversine_vector(
            XYPN[["X", "Y"]].iloc[i], XYPN[["X", "Y"]].iloc[:i], comb=True, check=False)
        L = []
        j = 0
