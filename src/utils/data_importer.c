...
void borne_data_importer(csv_import *file, borne_list *list_borne) {
  ...
  int length_of_line = line_getSize(*file->csv[0]);
  int id = 0;
  for (int i = 0; i < number_of_line; i++) {
    line *current_line = *file->csv[i]; //get the current line number i

    //create a new borne with data stored in csv_import(changing its type) and append it to the borne_list
    borne_list_append(list_borne, borne_create(id, atoi(current_line->args[2]),
                                               atoi(current_line->args[3]),
                                               atof(current_line->args[0]),
                                               atof(current_line->args[1])));
    id++;
  }
}
...
void car_data_importer(csv_import *file, car_list *list_car) {
  ...
  for (int i = 0; i < number_of_line; i++) {
    ...
    car_list_append(list_car, car_create(current_line->args[0], id,
                                   atoi(current_line->args[1]),
                                   atof(current_line->args[2])));
    id++;
  }
}
