...
bornes_graph *generate_graph_fromCSV(int autonomie,
  ...
  fclose(fp);
  time(&end1);
  printf("temps de création de la liste de toutes les bornes : %f\n",
  ...
  for (int i = 1; i < NB_BORNES - 1; i++) {
    if (i % 1000 == 0) {
      printf(" generation du graph : %f %% \n",
             (float)i / (float)NB_BORNES * 100.0);
    }
    for (int j = 0; j < i; j++) { // possible out of range
      int index = index_of_distance(
          i, j); // récupère l'indice de distance qui correspond à la distance
                 // entre les bornes i et j
      if (distances[index] > 0) { // si la distance est positive, on ajoute les
                                  // bornes i et j en voisin l'une de l'autre
        add_borne_index(bg, i,
                        get_borne(list_de_toutes_les_bornes,
                                  j)); // ajout de la borne i en voisin de j
        add_borne_index(bg, j,
                        get_borne(list_de_toutes_les_bornes,
                                  i)); // ajout de la borne j en voisin de i
      }
    }
  }
  time(&end);
  double duree = difftime(end, start);
  printf("Temps d'execution création graph : %f\n", duree);
  free(csvLine);
